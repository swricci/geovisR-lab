...
register_google(key='AIzaSyBwCEF31xRcLcLFZpRnOyXXSjGWYjCXf0E')

basemap <- ggmap(get_map(location = c(lon = -81.6, lat = 24.5),
                    zoom = 9, scale = 1,
                    maptype ='satellite'))

SPL.map<- basemap + geom_raster(data=boat_df, aes(x=x,y=y,fill = SPL,text=paste('SPL (dB):', round(SPL,2))),alpha = 0.8, inherit.aes = F)+
