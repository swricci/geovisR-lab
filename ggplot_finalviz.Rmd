...
SPL.map<- basemap + geom_raster(data=boat_df, aes(x=x,y=y,fill = SPL,text=paste('SPL (dB):', round(SPL,2))), inherit.aes = F)+
  scale_fill_viridis_c(option = "magma",na.value="transparent",alpha = 0.6)+
  geom_sf(data=fknms, aes(text=Name),color = "black",fill="gray",alpha=0.6,inherit.aes = FALSE)+
  theme_classic()+
  labs(
...
g<-ggplotly(SPL.map, tooltip = "text") #set height and width? adjust white space inside axes
#transparency???
...
Draft ggplotly object. The plot is built with three data layers: raster with sound levels, the state shapefile and the FKNMS zones shapefile. 
```{r, message =F, warning=F,eval=F}
SPL.map<-ggplot()+
...
Based on feedback, decided to add a basemap tile of satellite imagery. This gives more context to where the location is within the Florida Keys and provides some local bathymetry information. Bathymetry is an important factor when considering noise propagation so including this in the visualization should help with understanding the results.
