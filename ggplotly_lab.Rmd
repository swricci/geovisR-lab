...
To explore the functionality of ggplot, we will use the included diamonds data set. First, look at the structure of the data to see what types of variables we are working with.
...
Next, we will load and filter the data provided by Buzzfeed. This data preparation section will be used in other parts of the lab (plotly, shiny).  
```{r data.prep , eval = F}
...
For the remainder of the lab we will be working with the se_fires data set.
...
Construct a bubble plot (like the one from the original Buzzfeed article). Even with the reduced SE dataset the plot will take some time to run and render (~1 min).
```{r , eval = F}
